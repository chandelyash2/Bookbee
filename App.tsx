...
const App: React.FC = () => {
  const [view, setView] = useState<'home' | 'editor' | 'how-it-works' | 'pricing' | 'features' | 'about'>('home');
  ...
  const handleDirectExportPDF = async () => {
    ...
    try {
      ...
      const localStyles = Array.from(document.querySelectorAll('style')).map(el => el.outerHTML).join('\\n');
      const localLinks = Array.from(document.querySelectorAll<HTMLLinkElement>('link[rel="stylesheet"]'))
        .filter(el => !el.href.includes('fonts.googleapis.com'))
        .map(el => `<link rel="stylesheet" href="${new URL(el.getAttribute('href') || '', window.location.href).href}">`)
        .join('\\n');
...
<body>${clone.outerHTML}</body>
  ...
  if (view === 'home') {
    return <Home onStart={() => setView('editor')} onHowItWorks={() => setView('how-it-works')} onPricing={() => setView('pricing')} onFeatures={() => setView('features')} onAbout={() => setView('about')} />;
  }

  if (view === 'how-it-works') {
    return <HowItWorks onStart={() => setView('editor')} onHome={() => setView('home')} onPricing={() => setView('pricing')} onFeatures={() => setView('features')} onAbout={() => setView('about')} />;
  }
  
  if (view === 'pricing') {
    return <Pricing onStart={() => setView('editor')} onHome={() => setView('home')} onHowItWorks={() => setView('how-it-works')} onFeatures={() => setView('features')} onAbout={() => setView('about')} />;
  }

  if (view === 'features') {
    return <Features onStart={() => setView('editor')} onHome={() => setView('home')} onHowItWorks={() => setView('how-it-works')} onPricing={() => setView('pricing')} onAbout={() => setView('about')} />;
  }

  if (view === 'about') {
    return <About onStart={() => setView('editor')} onHome={() => setView('home')} onHowItWorks={() => setView('how-it-works')} onPricing={() => setView('pricing')} onFeatures={() => setView('features')} />;
  }
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 font-sans">
      ...
      <header className="h-16 bg-white border-b flex items-center justify-between px-8 sticky top-0 z-40 shadow-sm">
        <div className="flex items-center gap-2">
          <button className="md:hidden p-1 -ml-2 text-slate-600 hover:text-slate-900" onClick={() => setIsMobileMenuOpen(true)}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" /></svg>
          </button>
          <div className="cursor-pointer hover:opacity-80 transition hidden sm:block" onClick={() => setView('home')} title="Return to Home">
            <Logo />
          </div>
          <div className="cursor-pointer hover:opacity-80 transition sm:hidden" onClick={() => setView('home')} title="Return to Home">
             <span className="text-xl font-black tracking-tighter text-slate-800">Cover<span className="text-yellow-500">Bee</span></span>
          </div>
        </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-white/95 backdrop-blur-md flex flex-col items-center justify-center space-y-8 animate-in fade-in zoom-in-95 duration-200">
          ...
          <button 
            onClick={() => { setIsMobileMenuOpen(false); setView('features'); }} 
            className="text-2xl font-bold text-slate-800"
          >
            Features
          </button>
          <button 
            onClick={() => { setIsMobileMenuOpen(false); setView('how-it-works'); }} 
            className="text-2xl font-bold text-slate-800"
          >
            How It Works
          </button>
          <button 
            onClick={() => { setIsMobileMenuOpen(false); setView('pricing'); }} 
            className="text-2xl font-bold text-slate-800"
          >
            Pricing
          </button>
